...
func testAdminAuthConfigJSONForDaemon(t *testing.T, username string, password string) string {
	t.Helper()
	hash, err := hashDaemonAdminPassword(password, 4)
	if err != nil {
		t.Fatalf("hashDaemonAdminPassword failed: %v", err)
	}
	return fmt.Sprintf(`{"username":%q,"password_hash":%q}`, username, hash)
}

func TestLoadDaemonAdminAuthConfig(t *testing.T) {
	dir := t.TempDir()
	validPath := filepath.Join(dir, "admin_auth.json")
	if err := os.WriteFile(validPath, []byte(testAdminAuthConfigJSONForDaemon(t, "admin", "secret")), 0o644); err != nil {
	...
	if cfg.Username != "admin" || strings.TrimSpace(cfg.PasswordHash) == "" || cfg.Password != "" {
	...
	if err := os.WriteFile(emptyUsernamePath, []byte(`{"username":"","password_hash":"abc"}`), 0o644); err != nil {
	...
	if err := os.WriteFile(emptyPasswordPath, []byte(`{"username":"admin","password_hash":""}`), 0o644); err != nil {
...
func TestStartDaemonAdminServiceFallbackAndEndpoints(t *testing.T) {
	...
	authHash, err := hashDaemonAdminPassword(testDaemonAdminPassword, 4)
	if err != nil {
		t.Fatalf("hashDaemonAdminPassword failed: %v", err)
	}
	...
	legacyAdminBookingResp, err := httpGetWithBasicAuth(baseURL+bookingAdminStatusPath, authConfig.Username, testDaemonAdminPassword)
	if err != nil {
	...
	homeResp, err := httpGetWithBasicAuth(svc.URL(), authConfig.Username, testDaemonAdminPassword)
	if err != nil {
	...
	statusResp, err := httpGetWithBasicAuth(baseURL+daemonAdminStatusPath, authConfig.Username, testDaemonAdminPassword)
	if err != nil {
	...
	bookingServiceStatusResp, err := httpGetWithBasicAuth(baseURL+daemonAdminBookingServiceStatusPath, authConfig.Username, testDaemonAdminPassword)
	if err != nil {
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminTaskResumePath, url.Values{"id": {taskSnapshot.ID}}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminTaskPausePath, url.Values{"id": {taskSnapshot.ID}}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminMonitorStartPath, url.Values{"id": {"m-1"}}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminMonitorStopPath, url.Values{"id": {"m-1"}}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	if !cancelCalled {
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminMonitorStartAllPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	if startAllCalls == 0 {
		t.Fatalf("expected start-all callback called")
	}

	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminWebhookStartPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminWebhookStopPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminWebhookKillPortPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	...
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminBookingServiceStartPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	bookingOwnedMu.Lock()
	...
	bookingOwnedMu.Unlock()
	mustPOSTFormWithBasicAuth(t, baseURL+daemonAdminBookingServiceStopPath, url.Values{}, authConfig.Username, testDaemonAdminPassword, http.StatusOK)
	bookingOwnedMu.Lock()
	if _, tracked := bookingOwnedPIDs[12001]; tracked {
		bookingOwnedMu.Unlock()
	...
	getBookingActionResp, err := httpGetWithBasicAuth(baseURL+daemonAdminBookingServiceStartPath, authConfig.Username, testDaemonAdminPassword)
	if err != nil {
	...
	resp405, err := httpGetWithBasicAuth(baseURL+daemonAdminTaskPausePath, authConfig.Username, testDaemonAdminPassword)
	if err != nil {
...
func TestStartDaemonAdminServiceAllPortsBusy(t *testing.T) {
	...
	taskManager := newDaemonTaskManagerWithPaths(func(ctx context.Context, commands [][]string) error { return nil }, filepath.Join(t.TempDir(), "daemon_tasks.json"), filepath.Join(t.TempDir(), "daemon_tasks_history.json"))
	defer taskManager.Close()
	hash, err := hashDaemonAdminPassword(testDaemonAdminPassword, 4)
	if err != nil {
		t.Fatalf("hashDaemonAdminPassword failed: %v", err)
	}
