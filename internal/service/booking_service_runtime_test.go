...
func testAdminAuthConfigJSON(t *testing.T, username string, password string) string {
	t.Helper()
...
func testSecurityKeysConfigJSON(token string, scopes ...string) string {
	normalizedScopes := scopes
	if len(normalizedScopes) == 0 {
		normalizedScopes = []string{securityScopeBooking, securityScopeWebhook}
	}
...
func testAdminAuthConfig(t *testing.T, username string, password string) daemonAdminAuthConfig {
	t.Helper()
	hash, err := hashDaemonAdminPassword(password, 4)
	if err != nil {
		t.Fatalf("hash admin password failed: %v", err)
	}
	return daemonAdminAuthConfig{
		Username:     username,
		PasswordHash: hash,
	}
}

func setBookingSignedHeaders(req *http.Request, thirdPartyID string, token string, body []byte, now time.Time) {
...
func TestLoadSecurityTokenRecordsForBooking(t *testing.T) {
	dir := t.TempDir()
	cfgPath := filepath.Join(dir, "security_keys.json")
	if err := os.WriteFile(cfgPath, []byte(`{
  "version": 1,
  "tokens": [
    {"third_party_id":"partner-a","token":"token-a","scopes":["booking"]},
    {"third_party_id":"partner-b","token":"token-b","scopes":["booking","webhook"]}
  ]
}`), 0o644); err != nil {
		t.Fatalf("write security keys config failed: %v", err)
	}
	records, err := loadSecurityTokenRecords(cfgPath, false)
	if err != nil {
	...
	if err := os.WriteFile(emptyPath, []byte(`{"version":1,"tokens":[]}`), 0o644); err != nil {
		t.Fatalf("write empty security keys config failed: %v", err)
	}
	emptyRecords, err := loadSecurityTokenRecords(emptyPath, false)
	if err != nil {
		t.Fatalf("expected empty token list accepted, got %v", err)
	}
	if len(emptyRecords) != 0 {
		t.Fatalf("expected 0 records for empty token list, got %v", emptyRecords)
	}
}

func TestBookingPublicAuthSignedHeadersOnly(t *testing.T) {
	now := time.Now().UTC()
	...
	if controller.isAPIKeyAuthorized(priorityReq) {
		t.Fatalf("expected signed header validation failure")
	}
...
func TestBookingPublicAuthFailureReasons(t *testing.T) {
	...
	if !strings.Contains(strings.ToLower(noKeyBody), "missing required headers") {
		t.Fatalf("expected missing required headers reason, got %s", noKeyBody)
	}
...
func TestBookingServiceStartStopWithFallback(t *testing.T) {
	...
	if err := os.WriteFile(apiKeysPath, []byte(testSecurityKeysConfigJSON("test-key")), 0o644); err != nil {
...
func TestStartBookingHTTPServiceAuthAndIntentFlow(t *testing.T) {
	...
	if err := os.WriteFile(apiKeysPath, []byte(testSecurityKeysConfigJSON("booking-key")), 0o644); err != nil {
		t.Fatalf("write booking api keys failed: %v", err)
	}
	if err := os.WriteFile(llmConfigPath, []byte(`{"api_key":"sk-test","base_url":"https://api.openai.com"}`), 0o644); err != nil {
		t.Fatalf("write booking llm config failed: %v", err)
	...
	setBookingSignedHeaders(authCatalogReq, "partner-a", "booking-key", nil, time.Now().UTC())
	...
	setBookingSignedHeaders(reservationsReq, "partner-a", "booking-key", nil, time.Now().UTC())
...
func TestBookingAdminHomeSlashRedirectKeepsQuery(t *testing.T) {
	...
	controller := &bookingServiceController{
		...
		authConfig: testAdminAuthConfig(t, "admin", "secret"),
		nowFn:      time.Now,
	}
	publicMux := http.NewServeMux()
	adminMux := http.NewServeMux()
...
func TestBookingIntentParseLLMUnavailable(t *testing.T) {
	...
	setBookingSignedHeaders(catalogReq, "partner-a", "booking-key", nil, time.Now().UTC())
	...
	parseBodyJSON := `{"user_id":"u-1","content":"test"}`
...
func TestBookingServiceStatusAlreadyRunning(t *testing.T) {
	...
	if err := os.WriteFile(cfg.APIKeysPath, []byte(testSecurityKeysConfigJSON("k")), 0o644); err != nil {
		t.Fatalf("write api keys failed: %v", err)
	}
	if err := os.WriteFile(cfg.AdminAuthPath, []byte(testAdminAuthConfigJSON(t, "admin", "secret")), 0o644); err != nil {
		t.Fatalf("write admin auth failed: %v", err)
	}
